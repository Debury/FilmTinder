          ...
          <v-card class="pa-4 film-card" elevation="10">
            <v-list class="film-list">
              <v-list-item :key="film.id" class="py-3">
                ...
                <v-list-item-content>
                  <v-list-item-title class="text-h6 mb-1">{{ film.title }}</v-list-item-title>
...
export default {
  ...
  methods: {
    ...
    //David toto som sa snazil spravit
    compareMovies(){
      ...
      if(this.friend !== null){ 
        ...
        .then(result => {
          if (result == true) {
            ...
            console.log(`${movie.title} is in the collection.`);
          } else {
            // Not a match
            console.log(`${movie.title} is not in the collection.`);
